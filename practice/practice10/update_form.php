<?php
 ...
 $qmahasiswa = "SELECT mahasiswa.mahasiswa_id, mahasiswa.nama_lengkap, kelas.nama, mahasiswa.alamat, mahasiswa.created, mahasiswa.updated FROM mahasiswa INNER JOIN kelas ON mahasiswa.kelas_id=kelas.kelas_id;";
 $data_mahasiswa = $conn->query($qmahasiswa);
 $qjml_mahasiswa = "select count(mahasiswa_id) from mahasiswa";
 $jml_mahasiswa = $conn->query($qjml_mahasiswa);

 $qselect_mahasiswa = "select * from mahasiswa left join kelas on kelas.kelas_id = mahasiswa.kelas_id
    ...
    <div class="container">
      ...
      <div class="row">
        <div class="col-md-4 order-md-2 mb-4">
          Konten Data
          <?php
            foreach($jml_mahasiswa as $index => $value){
          ?>
          <h4 class="d-flex justify-content-between align-items-center mb-3">
            <span class="text-muted">Data Mahasiswa</span>
            <span class="badge bg-secondary badge-pill"><?php echo $value['count(mahasiswa_id)']; ?></span>
          </h4>
          <?php
            }
          ?>
